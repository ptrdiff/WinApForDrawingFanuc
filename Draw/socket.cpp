...
#define SENDBUF 34 //длина буфера для отправки
...
int socketmain(int argc, char* argv[])
{
	setlocale(LC_CTYPE, "Russian");

	//==================================================================================
	//char fname[] = "C:\\Cords\\inputhard.txt"; //человечек
	char fname[] = "D:\\qwerty.txt";
	//char fname[] = "heard.txt"; //сердечко
	int numOfCoord = 0;
	//char fname[] = "twol.txt";
	...
	*/
	char coord[4];
	sprintf_s(coord, "%d %d", syscoord, toolcoord);
	sendany(my_sock, coord);//отправляем буфер

	//=======================================================================================================================

	int image_ax, image_ay, image_az, image_bx, image_by, image_bz, image_cx, image_cy, image_cz; //координаты угловых точек при переносе картинки на доску

	double image_width = 0, image_high = 0;//ширина и высота картинки

	int center_x, center_y, center_z; //координаты центра доски/изображения

	//высчитываем координаты центра доски:
	center_x = (desk_ax - desk_cx) / 2 + desk_cx;
	center_y = (desk_ay - desk_cy) / 2 + desk_cy;
	center_z = (desk_az - desk_cz) / 2 + desk_cz;
	...
	double att_x, att_y;//отношения по ширине и высоте

	int x, y;//X и Y точки на картинке
...
//================================================================================================
	...
	*/
	...
	int dx = (int)(nx / mod * 100);//смещение
	int dy = (int)(ny / mod * 100);
	int dz = (int)(nz / mod * 100);
	...
	while (inFile.eof() == false) {
		...
		for (int i = 0; (inFile.get() != '|' && inFile.eof() == false); ++i) {
			inFile >> x >> y; //надо считать x и y и записать их и в переменные для дальнейшего расчета.
			std::cout << x << " " << y << " | ";
			att_y = 1 - (x / image_width);//высчитываем отношение растояния до точки от 0 к длине стороны
			att_x = 1 - (y / image_high);//высчитываем отношение растояния до точки от 0 к длине стороны
//--------------------------------------
			Xr = (int)(((image_width*att_x) + image_cx)*(nz / sqrt(nz * nz + ny * ny)));//прибавляем к получившимся координатам по x дефолтное значение для того, чтобы рисунок был на доске,
												 // а не около самого робота, т.к. x=0 у основания
			Yr = (int)(((image_high*att_y) + image_cy)*(nz / sqrt(nz * nz + nx * nx)));//вычитаем из получившихся координат по y дефолтное значение для того, чтобы рисунок был по-середине, а не только
...
//--------------------------------------
			...
			if (i == 0) {	//Если первая точка контура, то дополнительно отправить координаты для поднятия маркера;
				resbuf.push(Xr + dx - 20);
				resbuf.push(Yr + dy - 72);
				resbuf.push(Z + dz - 190);
				resbuf.push(UW);
				resbuf.push(UP);
				resbuf.push(uz);
				resbuf.push(segtime);
			...
			for (size_t j = 0; j < resbuf.size(); ++j) {
				...
				resbuf.pop();
				sprintf_s(sendbuf, "%d %d %d %d %d %d %d %d", tmpX, tmpY, tmpZ, tmpW, tmpP, tmpR, tmpSeg, tmpT);
